...
export default function FooterMenus({
  ...
  return (
    <div className="from-lavender bg-gradient-to-b">
      <div className="container md:flex md:flex-wrap md:mb-6 py-8 gap-8">
        <menu className="flex-grow-0 flex-shrink basis-[350px] mb-6 md:mb-0">
          <Accordion header={<h4>Newsletter</h4>}>
        ...
        {menu?.items.map((menu, index) => (
          ...
          >
            <Accordion header={<h4>{menu.title}</h4>} className="mt-6 md:mt-0">
              <ul className="flex flex-col gap-1 mt-5">
                {menu.items.map((item, index) => (
                  <li key={`sub-menu-${index}`} className="m-0">
                    <Link
                      href={item?.path}
                      className="no-underline text-tertiary hover:text-primary"
                    >
                      {item.title}
                    </Link>
                  </li>
        ...
        <menu className="flex-grow flex-shrink basis-[350px] max-w-[460px] md:border-none border-t-[1px] border-[#BEBBC4] mt-6 md:mt-0">
